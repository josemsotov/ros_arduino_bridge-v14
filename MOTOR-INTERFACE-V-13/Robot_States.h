...
// Flags de dirección para ISR Hall (definidos en Motor_Control.h)
...
void moveForwardState(int pwm) {
  ...
  // *** ACTUALIZAR FLAGS DE DIRECCIÓN PRIMERO ***
  motorGoingForwardLeft  = true;
  motorGoingForwardRight = true;

  DEBUG_PRINT("ADELANTE a PWM=");
  DEBUG_PRINTLN(pwm);
  
  // *** PASO 1: STOP=LOW y BRAKE=FLOAT ***
  // El ZS-X11H jala DIR a GND mientras STOP=LOW, por eso NO escribimos DIR todavía.
  analogWrite(PWM_LEFT_MOTOR, 0);
  analogWrite(PWM_RIGHT_MOTOR, 0);
  ...
  // *** PASO 2: STOP=INPUT (float) — driver deja libre el pin DIR ***
  pinMode(STOP_LEFT_MOTOR, INPUT);
  pinMode(STOP_RIGHT_MOTOR, INPUT);
  delay(5);
  
  // *** PASO 3: DIR AHORA — con STOP ya flotante, el driver no interfiere ***
  digitalWrite(DIR_LEFT_MOTOR, dirForwardLeft);    // adelante motor izquierdo
  digitalWrite(DIR_RIGHT_MOTOR, dirForwardRight);  // adelante motor derecho
  // (flags ya establecidos al inicio de la función)
  delay(10);  // Setup time del driver
  ...
  // Actualizar estado
  currentRobotState = STATE_HABILITADO;
  leftMotor.enabled  = true;  rightMotor.enabled  = true;
  leftMotor.stopped  = false; rightMotor.stopped  = false;
  leftMotor.braked   = false; rightMotor.braked   = false;
...
void moveBackwardState(int pwm) {
  ...
  // *** ACTUALIZAR FLAGS DE DIRECCI\u00d3N PRIMERO (antes de cualquier cambio de hardware) ***
  // Esto asegura que si alg\u00fan pulso Hall dispara durante la transici\u00f3n,
  // ya se cuenta como direcci\u00f3n atr\u00e1s.
  motorGoingForwardLeft  = false;
  motorGoingForwardRight = false;
  ...
  // *** PASO 3: DIR AHORA — con STOP ya flotante, el driver no interfiere ***
  digitalWrite(DIR_LEFT_MOTOR, !dirForwardLeft);    // atrás motor izquierdo
  digitalWrite(DIR_RIGHT_MOTOR, !dirForwardRight);  // atrás motor derecho
  // (flags ya establecidos al inicio de la funci\u00f3n)
  delay(10);  // Setup time del driver
...
void executeCruceDerecha(int pwm, uint16_t pulses) {
  ...
  digitalWrite(DIR_LEFT_MOTOR, dirForwardLeft);
  motorGoingForwardLeft = true;   // flag para ISR Hall
...
void executeCruceIzquierda(int pwm, uint16_t pulses) {
  ...
  digitalWrite(DIR_RIGHT_MOTOR, dirForwardRight);
...
void printRobotState() {
  DEBUG_PRINT("Estado actual del robot: ");
  DEBUG_PRINTLN(getRobotStateName());
  
  switch(currentRobotState) {
    case STATE_INHABILITADO:
      DEBUG_PRINTLN("  STOP=LOW, BRAKE=FLOAT, PWM=0");
      DEBUG_PRINTLN("  Motores deshabilitados");
      break;
    case STATE_BLOQUEADO:
      DEBUG_PRINTLN("  STOP=LOW, BRAKE=HIGH, PWM=0");
      DEBUG_PRINTLN("  Motores bloqueados");
      break;
    case STATE_HABILITADO:
      DEBUG_PRINTLN("  STOP=FLOAT, BRAKE variable, listo para comandos");
      DEBUG_PRINTLN("  Comandos disponibles: ADELANTE, ATRAS, AD, AT, CRUCE-DER, CRUCE-IZQ");
      break;
  }
}
