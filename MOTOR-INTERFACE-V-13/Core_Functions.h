 ...
 */
  ...
  volatile int32_t  leftHallSigned  = 0;   // Contador con signo (+adelante, -atrás)
  volatile int32_t  rightHallSigned = 0;   // Contador con signo (+adelante, -atrás)
#endif
...
void initializeSystem() {
  ...
  // Banner de versión — siempre visible (usa Serial directo, no DEBUG_PRINTLN)
  Serial.println(F("========================================"));
  Serial.print(F("  arduino_bridge "));
  Serial.println(F(FIRMWARE_VERSION_STR));
  Serial.print(F("  Modo: "));
  #if defined(MODO_BANCO_ESTATICO)
    Serial.println(F("BANCO ESTATICO (sin MPU)"));
  #elif defined(MODO_BANCO_DINAMICO)
    Serial.println(F("BANCO DINAMICO (Hall + MPU)"));
  #elif defined(MODO_OPERATIVO)
    Serial.println(F("OPERATIVO"));
  #endif
  Serial.println(F("========================================"));

  // Cargar configuracion de direccion desde EEPROM
  // Dirección EEPROM 0x10 = dirForwardLeft, 0x11 = dirForwardRight
  // Firma 0xAC en 0x12 = datos válidos (0xAC desde 2026-04-24 tras verificar dirForwardLeft=LOW)
  if (EEPROM.read(0x12) == 0xAC) {
    dirForwardLeft  = (bool)EEPROM.read(0x10);
    dirForwardRight = (bool)EEPROM.read(0x11);
    Serial.print("[EEPROM] DIR cargados: IZQ=");
    Serial.print(dirForwardLeft  ? "HIGH(1)" : "LOW(0)");
    Serial.print(" DER=");
    Serial.println(dirForwardRight ? "HIGH(1)" : "LOW(0)");
  } else {
    // Firma no reconocida: guardar defaults verificados 2026-04-24
    // dirForwardLeft=LOW, dirForwardRight=LOW (ambos pines mismo nivel para ADELANTE)
    EEPROM.write(0x10, (uint8_t)dirForwardLeft);
    EEPROM.write(0x11, (uint8_t)dirForwardRight);
    EEPROM.write(0x12, 0xAC);
    Serial.println("[EEPROM] Defaults escritos: IZQ=HIGH(1) DER=LOW(0)");
  }
  ...
  // Inicializar odometría si está habilitada
  ...
  // Inicializar Bluetooth si está habilitado
  #ifdef ENABLE_BLUETOOTH
    bt_initialize();
  #endif

  // Inicializar GPS si está habilitado
  #ifdef ENABLE_GPS
    gps_initialize();
  #endif
