...
void processSerialCommand() {
  ...
  // ===== COMANDO DE PRUEBA DIRECTA DE POSICIÓN (BYPASS PID) =====
    ...
    // Usar lógica correcta de dirección (misma polaridad ambos motores)
    setLeftMotor(50, true);   // true = HIGH = adelante para motor izquierdo
    setRightMotor(50, true);  // true = HIGH = adelante para motor derecho
  ...
  // ===== COMANDO DE PRUEBA VELOCIDAD CONTINUA (BYPASS PID) =====
    ...
    setRightMotor(30, true);  // true = HIGH = adelante para motor derecho
  ...
  // ===== COMANDOS DE MOVIMIENTO CON CONTROL DE ESTADO =====
    ...
    Serial.print(">> ADELANTE PWM=");
    Serial.print(DEFAULT_PWM_FORWARD);
    Serial.print(" DIR_IZQ(pin)=");
    Serial.print(digitalRead(DIR_LEFT_MOTOR));
    Serial.print(" DIR_DER(pin)=");
    Serial.println(digitalRead(DIR_RIGHT_MOTOR));
  }
    ...
    Serial.print(">> ATRAS PWM=");
    Serial.print(DEFAULT_PWM_BACKWARD);
    Serial.print(" DIR_IZQ(pin)=");
    Serial.print(digitalRead(DIR_LEFT_MOTOR));
    Serial.print(" DIR_DER(pin)=");
    Serial.println(digitalRead(DIR_RIGHT_MOTOR));
  ...
  // ===== CONFIGURACIÓN DE DIRECCIÓN EN RUNTIME =====
  // DIR_IZQ 1 → pin DIR izquierdo en HIGH = adelante
  // DIR_IZQ 0 → pin DIR izquierdo en LOW  = adelante
  // DIR_DER 1 → pin DIR derecho  en HIGH = adelante
  // DIR_DER 0 → pin DIR derecho  en LOW  = adelante
  else if (cmd.startsWith("DIR_IZQ")) {
    int val = extractNumber(cmd, -1);
    if (val == 0 || val == 1) {
      dirForwardLeft = (bool)val;
      EEPROM.write(0x10, (uint8_t)dirForwardLeft);
      EEPROM.write(0x12, 0xAC);
      Serial.print(">> DIR_IZQ guardado en EEPROM: ");
      Serial.print(dirForwardLeft ? "HIGH (1)" : "LOW (0)");
      Serial.println(" = adelante motor izquierdo");
    } else {
      Serial.print(">> DIR_IZQ actual: ");
      Serial.println(dirForwardLeft ? "HIGH (1)" : "LOW (0)");
      Serial.println("   Uso: DIR_IZQ 0  o  DIR_IZQ 1");
    }
  }
  else if (cmd.startsWith("DIR_DER")) {
    int val = extractNumber(cmd, -1);
    if (val == 0 || val == 1) {
      dirForwardRight = (bool)val;
      EEPROM.write(0x11, (uint8_t)dirForwardRight);
      EEPROM.write(0x12, 0xAC);
      Serial.print(">> DIR_DER guardado en EEPROM: ");
      Serial.print(dirForwardRight ? "HIGH (1)" : "LOW (0)");
      Serial.println(" = adelante motor derecho");
    } else {
      Serial.print(">> DIR_DER actual: ");
      Serial.println(dirForwardRight ? "HIGH (1)" : "LOW (0)");
      Serial.println("   Uso: DIR_DER 0  o  DIR_DER 1");
    }
  }
  else if (cmd.startsWith("L")) {
    int pwm = extractNumber(cmd, 0);
    setLeftMotor(pwm, true);   // true = adelante
  }
  else if (cmd.startsWith("R")) {
    int pwm = extractNumber(cmd, 0);
    setRightMotor(pwm, true);  // true = adelante
  }
  ...
  else if (cmd == "HALL_FLAGS" || cmd == "HALL_DIR") {
    Serial.println("=== FLAGS DIRECCI\u00d3N (ISR Hall) ===");
    Serial.print("  motorGoingForwardLeft  : ");
    Serial.println(motorGoingForwardLeft  ? "true  (ADELANTE)" : "false (ATRAS)");
    Serial.print("  motorGoingForwardRight : ");
    Serial.println(motorGoingForwardRight ? "true  (ADELANTE)" : "false (ATRAS)");
    Serial.print("  leftHallSigned         : "); Serial.println(leftHallSigned);
    Serial.print("  rightHallSigned        : "); Serial.println(rightHallSigned);
    Serial.print("  leftHallCount (total)  : "); Serial.println(leftHallCount);
    Serial.print("  rightHallCount (total) : "); Serial.println(rightHallCount);
    Serial.println("======================================");
  }
  #endif

  // ===== COMANDOS MPU9250/6500 =====
  #ifdef ENABLE_MPU9250
  else if (cmd == "MPU_STATUS" || cmd == "MPU") {
    mpu_printStatus();
  }
  else if (cmd == "MPU_CALIBRATE" || cmd == "MPU_CAL") {
    Serial.println(">> Calibrando MPU... (manten el robot quieto)");
    mpu_calibrateOffsets();
  }
  else if (cmd == "MPU_RESET" || cmd == "MPU_ZERO") {
    mpu_resetYaw();
    Serial.println(">> Yaw reseteado a 0°");
  }
  else if (cmd == "MPU_REF") {
    mpu_setYawReference();
    Serial.print(">> Referencia Yaw establecida en: ");
    Serial.print(mpu_getYaw(), 2);
    Serial.println("°");
  }
  else if (cmd == "MPU_DEBUG") {
    #ifdef ENABLE_MPU_DEBUG
      mpu_continuousDebug = !mpu_continuousDebug;
      Serial.print(">> MPU debug continuo: ");
      Serial.println(mpu_continuousDebug ? "ACTIVADO" : "DESACTIVADO");
    #else
      Serial.println(">> Activa ENABLE_MPU_DEBUG en Configuration.h para usar MPU_DEBUG");
    #endif
  }
  #endif

  // ===== COMANDOS ODOMETRÍA =====
  #if defined(ENABLE_ODOMETRY) && defined(ENABLE_HALL_SENSORS)
  else if (cmd == "ODOM" || cmd == "ODOM_STATUS") {
    odom_printStatus();
  }
  else if (cmd == "ODOM_RESET" || cmd == "ODOM_ZERO") {
    odom_reset();
  }
  else if (cmd == "ODOM_DEBUG") {
    odom_debugActive = !odom_debugActive;
    Serial.print(">> Odometría debug continuo: ");
    Serial.println(odom_debugActive ? "ACTIVADO" : "DESACTIVADO");
  }
  #endif

  // ===== COMANDOS BLUETOOTH HC-05 =====
  #ifdef ENABLE_BLUETOOTH
  else if (cmd == "BT_STATUS" || cmd == "BT") {
    bt_printStatus();
  }
  else if (cmd == "USE_BT") {
    #ifdef ENABLE_ROS2_BRIDGE
  ...
  else if (cmd == "BT_AT_CONFIG") {
    #ifdef ENABLE_BT_AT_COMMANDS
      bt_configAT();
    #else
      Serial.println(F(">> Activa ENABLE_BT_AT_COMMANDS en Configuration.h"));
    #endif
  }
  #endif
