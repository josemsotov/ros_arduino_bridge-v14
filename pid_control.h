...
// Mantener variables para compatibilidad con comando 'k' (ahora cambia speed-match).
float Kp_v = 0.25f, Ki_v = 0.0f, Kd_v = 0.0f; // PI velocidad V1; activacion explicita con k on
...
// =================== RESET PID ===================
// Llamar siempre que el setpoint sea 0 o cambie de sentido.
// Con Ki=0 y Kd=0 solo limpia prev_error (que no afecta), pero es
// indispensable para futuras ganancias no nulas.
void pid_reset_velocity() {
  pid_v_integral    = 0;  pid_v_prev_error  = 0;
  pid_w_integral    = 0;  pid_w_prev_error  = 0;
  pid_last_time     = 0;  // forzar recalculo de dt en proxima llamada
}
...
void pid_control_velocity(float v_ref, float w_ref, float v_meas, float w_meas, int &pwm_left, int &pwm_right) {
  ...
  // FIX Bug#4: primera llamada con pid_last_time=0 generaria dt = tiempo_arranque
  if (pid_last_time == 0) pid_last_time = now;
  float dt = (now - pid_last_time) / 1000.0f;
  if (dt <= 0.0f || dt > 0.1f) dt = 0.01f;  // clamp: evita spike de integral (max 100ms)
  pid_last_time = now;
  ...
  // Convertir a PWM con signo (positivo = adelante, negativo = atras)
  // El llamador extrae dir = (pwm >= 0) y aplica abs(pwm)
  pwm_left  = constrain((int)(v_left  * 100), -MAX_PWM_VALUE, MAX_PWM_VALUE);
  pwm_right = constrain((int)(v_right * 100), -MAX_PWM_VALUE, MAX_PWM_VALUE);
}
