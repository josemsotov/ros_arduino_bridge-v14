...
class OpenPalmNode(Node):
    ...
    def _trigger(self, gesture, now):
        ...
        if now - self.last_trigger < self.get_parameter('cooldown').value:
            return
        # Manual Stadia control has priority over passive camera gestures.
        # Without this gate, a MediaPipe false positive (especially
        # thumb_down) can continuously publish STOP after the controller has
        # been armed, making a healthy Stadia / Arduino chain look dead.
        # Explicitly armed body/hand tests keep their own gesture semantics.
        if self.stadia_connected and self.stadia_mode == 'stadia' and not (
            self.body_field_armed or self.tilt_calibration_armed
        ):
            return
