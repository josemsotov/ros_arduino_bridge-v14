...
class FollowerNode(Node):
    ...
    def enable_cb(self, msg: Bool):
        ...
        # StadiaNode publishes both authorized_enable=False and enable=False
        # when manual mode is selected. Those messages may also be repeated.
        # Once the follower is already inactive, publishing another zero Twist
        # here races with and overwrites the manual Stadia /cmd_vel stream.
        # The transition into manual mode is already fail-closed by
        # StadiaNode._send_stop(); subsequent False messages are idempotent.
        if not requested and not self.enabled and self.manual_override_active:
            if not self.face_static_enabled:
                self.mode = 'WAITING'
            self.identity_enroll_active = False
            return
        if requested and self.manual_override_active:
            self.enabled = False
            self.mode = 'WAITING'
            self._stop()
