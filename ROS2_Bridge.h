...
static bool velocity_pi_enabled = false;
static float velocity_pi_corr_left = 0.0f;
...
void ros2_processCmdVel(String cmd) {
  ...
  if (space1 > 0 && space2 > 0) {
    ...
    // Telemetría compacta: se emite al final del procesamiento (ver línea T)
      ...
      float velocity_pi_dt = pid_per_wheel_last_time == 0 ? 0.1f :
        constrain((velocity_pi_now - pid_per_wheel_last_time) / 1000.0f, 0.02f, 0.20f);
      pid_per_wheel_last_time = velocity_pi_now;
      velocity_pi_corr_left = 0.0f;
      velocity_pi_corr_right = 0.0f;
      ...
      #endif
      {
        ...
        Serial.print(" Rpi="); Serial.print(velocity_pi_corr_right, 2);
        Serial.print(" PI="); Serial.print(velocity_pi_enabled ? 1 : 0);
...
void ros2_processCommand(String cmd) {
  ...
  switch (command) {
    ...
    case 'k': {
      // k on|off | k <kp_pwm_per_rpm> [ki_pwm_per_rpm_s]
      String params = cmd.substring(1);
      params.trim();
      if (params == "on") {
        velocity_pi_enabled = true;
        pid_per_wheel_reset();
      } else if (params == "off") {
        ...
        if (sp2 > 0) {
          Kp_v = constrain(params.substring(0, sp2).toFloat(), 0.0f, 2.0f);
          Ki_v = constrain(params.substring(sp2 + 1).toFloat(), 0.0f, 1.0f);
        } else {
          Kp_v = constrain(params.toFloat(), 0.0f, 2.0f);
        }
        pid_per_wheel_reset();
      }
